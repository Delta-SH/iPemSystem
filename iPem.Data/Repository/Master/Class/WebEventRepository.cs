            ...
            using (var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach (var entity in entities) {
                        ...
                        SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_WebEvent_Repository_Insert, parms);
                    }
                    trans.Commit();
                } catch {
                    trans.Rollback();
        ...
        public virtual void Delete(IList<WebEvent> entities) {
            ...
            using (var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach (var entity in entities) {
                        parms[0].Value = SqlTypeConverter.DBNullGuidChecker(entity.Id);
                        SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_WebEvent_Repository_Delete, parms);
                    }
                    trans.Commit();
                } catch {
                    trans.Rollback();
