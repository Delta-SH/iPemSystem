...
namespace iPem.Data.Repository.Master {
    public partial class OperateInRoleRepository : IOperateInRoleRepository {
        ...
        public virtual OperateInRole GetEntity(Guid role) {
            ...
            var entity = new OperateInRole { RoleId = role, OperateIds = new List<EnmOperation>() };
            using(var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Cs.Sql_OperateInRole_Repository_GetEntities, parms)) {
                while(rdr.Read()) {
                    entity.OperateIds.Add(SqlTypeConverter.DBNullEnmOperationHandler(rdr["OperateId"]));
                }
            }
            return entity;
        }
            ...
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach(var entity in entities) {
                        ...
                        foreach(var id in entity.OperateIds) {
                            parms[1].Value = (Int32)id;
                            SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_OperateInRole_Repository_Insert, parms);
                        }
                    }
                    trans.Commit();
                } catch {
            ...
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach(var id in roles) {
                        ...
                        SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_OperateInRole_Repository_Delete, parms);
                    }
                    trans.Commit();
                } catch {
                    trans.Rollback();
