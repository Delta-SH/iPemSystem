...
namespace iPem.Data.Repository.Master {
    public partial class MenusInRoleRepository : IMenusInRoleRepository {
        ...
        public virtual MenusInRole GetEntity(Guid role) {
            ...
            using(var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Cs.Sql_MenusInRole_Repository_GetEntities, parms)) {
                while(rdr.Read()) {
                ...
                try {
                    foreach(var entity in entities) {
                        ...
                        foreach(var menu in entity.Menus) {
                            ...
                            SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_MenusInRole_Repository_Insert, parms);
                        }
                    }
                    trans.Commit();
                } catch {
            ...
            using(var conn = new SqlConnection(this._databaseConnectionString)) {
                ...
                try {
                    foreach(var id in roles) {
                        ...
                        SqlHelper.ExecuteNonQuery(trans, CommandType.Text, SqlCommands_Cs.Sql_MenusInRole_Repository_Delete, parms);
                    }
                    trans.Commit();
                } catch {
                    trans.Rollback();
