...
namespace iPem.Data.Repository.Cs {
    public partial class V_BatTimeRepository : IV_BatTimeRepository {
        ...
        public List<V_BatTime> GetValuesInDevice(string id, DateTime start, DateTime end) {
            ...
            using (var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Cs.Sql_V_BatTime_Repository_GetValuesInDevice, parms)) {
                while (rdr.Read()) {
        ...
        public List<V_BatTime> GetValuesInPoint(string device, string point, DateTime start, DateTime end) {
            ...
            var entities = new List<V_BatTime>();
            using (var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Cs.Sql_V_BatTime_Repository_GetValuesInPoint, parms)) {
                while (rdr.Read()) {
                    var entity = new V_BatTime();
                    entity.AreaId = SqlTypeConverter.DBNullStringHandler(rdr["AreaId"]);
                    entity.StationId = SqlTypeConverter.DBNullStringHandler(rdr["StationId"]);
        ...
        public List<V_BatTime> GetValues(DateTime start, DateTime end) {
            ...
            var entities = new List<V_BatTime>();
            using (var rdr = SqlHelper.ExecuteReader(this._databaseConnectionString, CommandType.Text, SqlCommands_Cs.Sql_V_BatTime_Repository_GetValues, parms)) {
                while(rdr.Read()) {
                    var entity = new V_BatTime();
                    entity.AreaId = SqlTypeConverter.DBNullStringHandler(rdr["AreaId"]);
                    entity.StationId = SqlTypeConverter.DBNullStringHandler(rdr["StationId"]);
