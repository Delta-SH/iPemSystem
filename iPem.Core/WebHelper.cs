    ...
    public partial class WebHelper : IWebHelper {
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool useSsl) {
            ...
            if(includeQueryString) {
                string storeHost = GetAppAuthority(useSsl);
        ...
        /// <summary>
        /// Gets application url authority
        /// </summary>
        /// <returns>Application url authority</returns>
        public virtual string GetAppAuthority() {
            bool useSsl = IsCurrentConnectionSecured();
            return GetAppAuthority(useSsl);
        }
        ...
        /// <returns>Application url authority</returns>
        public virtual string GetAppAuthority(bool useSsl) {
            var httpHost = ServerVariables("HTTP_HOST");
            if(String.IsNullOrWhiteSpace(httpHost))
                httpHost = _httpContext.Request.Url.Authority;

            if(String.IsNullOrWhiteSpace(httpHost))
                throw new Exception("Http host cannot be found");

            var result = (useSsl ? "https://": "http://") + httpHost;
            if(!result.EndsWith("/"))
                result += "/";
        ...
        /// <summary>
        /// Gets application host
        /// </summary>
        /// <returns>Application host</returns>
        public virtual string GetAppHost() {
            bool useSsl = IsCurrentConnectionSecured();
            return GetAppHost(useSsl);
        }
        ...
        public virtual string GetAppHost(bool useSsl) {
            ...
            var httpHost = _httpContext.Request.Url.Host;
        ...
        /// </summary>
        /// <returns>Application port</returns>
        public virtual int GetAppPort() {
            if(!IsRequestAvailable(_httpContext))
                throw new Exception("Http host cannot be found");

            return _httpContext.Request.Url.Port;
        }

        /// <summary>
        /// Gets application location
        /// </summary>
        /// <returns>Application location</returns>
        ...
        public virtual string GetAppLocation(bool useSsl) {
            string result = GetAppAuthority(useSsl);
            if(result.EndsWith("/"))
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "") {
            if(CommonHelper.GetTrustLevel() > AspNetHostingPermissionLevel.Medium) {
                ...
                if(!success) {
                    throw new iPemException("we needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone {
            get {
                if(_httpContext.Items["iPem.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["iPem.IsPOSTBeingDone"]);
            }
            set {
                _httpContext.Items["iPem.IsPOSTBeingDone"] = value;
            }
        }
