	...
	public partial class FtpClient : IFtpClient, IDisposable {
		...
		public bool FileExists(string path) {
			...
			lock (m_lock) {
                ...
                // string[] fileList = GetListing(path.GetFtpDirectoryName()).Select(f => f.Name).ToArray();
                string[] fileList = GetFileNames(path.GetFtpDirectoryName());
        ...
        private string[] GetFileNames(string path) {
            try {
                return GetNameListing(path);
            } catch {
                return GetListing(path).Select(f => f.Name).ToArray();
            }
        }
