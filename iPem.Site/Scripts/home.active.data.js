        ...
        { text: '测值时间', dataIndex: 'time', width: 150 },
        {
            ...
            items: [{
                ...
                handler: function (view, rowIndex, colIndex, item, event, record) {
                    ...
                    control(record.get('deviceid'), record.get('device'), record.get('pointid'), record.get('point'));
                }
            }, {
                tooltip: '遥调',
                getClass: function (v, metadata, r, rowIndex, colIndex, store) {
                ...
                handler: function (view, rowIndex, colIndex, item, event, record) {
                    view.getSelectionModel().select(record);
                    adjust(record.get('deviceid'), record.get('device'), record.get('pointid'), record.get('point'));
                }
            }, {
...
var viewIFrame = Ext.create('Ext.ux.IFrame', {
    flex: 1,
    loadMask: '正在处理...',
    src: '/WGViewPlayer/GVPlayer.xbap'
});
...
var controlWnd = Ext.create('Ext.window.Window', {
    ...
    listeners: {
        beforehide: function (me) {
            if (currentTab() !== viewPanel) return;
            viewIFrame.show();
        },
...
var viewer = function (node) {
    ...
    if (current !== viewPanel) return false;
...
var control = function (deviceid, devicename, pointid, pointname) {
    var form = controlWnd.getComponent('controlForm'),
                device = form.getComponent('device'),
                point = form.getComponent('point'),
                name = form.getComponent('name'),
                ctrl = form.getComponent('ctrl'),
                result = Ext.getCmp('controlResult');

    device.setValue(deviceid);
    point.setValue(pointid);
    name.setRawValue(pointname);
    ctrl.bind(pointid);//请求信号的控制选项
    result.setTextWithIcon('', '');
    controlWnd.show();
};

var adjust = function (deviceid, devicename, pointid, pointname) {
    var form = adjustWnd.getComponent('adjustForm'),
                device = form.getComponent('device'),
                point = form.getComponent('point'),
                name = form.getComponent('name'),
