...
namespace iPem.Site.Infrastructure
{
    public class ApiWorkContext : IApiWorkContext
    {
        ...
        public List<SSHArea> GetAreas(string id)
        {
            ...
            var auths = this.GetAuth(id).Authorizations;
            if (auths.Count == 0) return _cachedAreas;

            var roleType = this.GetRole(id).Type;
            ...
            {
                var areaIds = new HashSet<string>(this.GetStations(id).Select(s => s.AreaId));
                foreach (var area in allAreas)
                {
            ...
            {
                var areaIds = new HashSet<string>(this.GetRooms(id).Select(r => r.AreaId));
                foreach (var area in allAreas)
                {
                    if (!areaIds.Contains(area.Id)) continue;
                    GetParents(allAreas, parents, area);
            ...
            {
                var areaIds = new HashSet<string>(this.GetDevices(id).Select(d => d.AreaId));
                foreach (var area in allAreas)
                {
                    if (!areaIds.Contains(area.Id)) continue;
                    GetParents(allAreas, parents, area);
        ...
        public List<S_Station> GetStations(string id)
        {
            ...
            {
                var stationIds = new HashSet<string>(this.GetRooms(id).Select(r => r.StationId));
                foreach (var station in allStations)
                {
            ...
            {
                var stationIds = new HashSet<string>(this.GetDevices(id).Select(d => d.StationId));
                foreach (var station in allStations)
                {
                    if (!stationIds.Contains(station.Id)) continue;
                    _cachedStations.Add(station);
        ...
        public List<S_Room> GetRooms(string id)
        {
            ...
            {
                var roomIds = new HashSet<string>(this.GetDevices(id).Select(d => d.RoomId));
                foreach (var room in allRooms)
                {
        ...
        public List<D_Fsu> GetFsus(string id)
        {
            ...
            {
                var fsuIds = new HashSet<string>(this.GetDevices(id).Select(d => d.FsuId));
        ...
        public List<AlmStore<A_AAlarm>> ActAlarms(string id)
        {
            ...
            if (_cachedActAlarms.Count > 0 && !U_Role.SuperId.Equals(id))
            {
                var keys = this.GetAuth(id).Authorizations;
                var roleType = this.GetRole(id).Type;
                if (roleType == EnmSSH.Area)
